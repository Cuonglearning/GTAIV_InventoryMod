...
namespace Inventory
{
    public class Inventory : Script
    {
        ...
        private readonly List<string> page1_items = new List<string> { "canned_soft_drink", "small_cola", "sprite_small", "coffee", "chocolate", "nuts", "packed_apple", "milk" };
        ...
        private bool IsPlayerFree()
        {
            ...
            if (!Player.CanControlCharacter ||
                ...
                playerPed.isShooting ||
                Game.isGameKeyPressed(GameKey.Attack) ||
                ((Game.isGameKeyPressed(GameKey.Jump) || (Game.isGameKeyPressed(GameKey.Reload) || Game.isGameKeyPressed(GameKey.Aim))) && !playerPed.isInVehicle()))
            {
                return false;
            }
            ...
            var animationSetDurations = new Dictionary<string, int>
    {
        ...
        private void InitializeCombineDatabase()
        {
            combineDatabase = new Dictionary<string, CombineInfo>
            {
                ...
                { "sandwich_special", new CombineInfo { Name = "Sandwich Special", Ingredients = new Dictionary<string, int> { {"hotdog", 1}, {"ciabatta", 1}, {"canned_soft_drink", 1} }, HealthCoreRestore = 89,GrantsGoldCore = true, AnimationDuration = 9000 } },
            ...
            if (canControl)
            {
                ...
                else
                {
                    if (currentMinute != lastHealthRegenGameMinute)
                    {
                        ...
                        if (healthRegenGameMinutesElapsed >= 6)
                        {
        ...
        private void HandleDeadeye()
        {
            ...
            if (isDeadeyeActive)
            {
                ...
                if ((DateTime.Now - lastDeadeyeDrainTime).TotalSeconds >= 1.5)
                {
                    deadeyeCore = Math.Max(0, deadeyeCore - 5);
        ...
        private bool HasValidWeapon(Ped player)
        {
            try
            {
                if (player.Weapons != null && player.Weapons.Current != null)
                {
                    ...
                    || player.Weapons.Current == Weapon.Misc_Object || player.Weapons.Current == Weapon.TLAD_Poolcue || player.Weapons.Current == Weapon.TBOGT_StickyBomb || player.Weapons.Current == Weapon.TLAD_PipeBomb)
                    {
                        return false;
                    }
        ...
        private void ToggleDeadeye()
        {
            ...
            {
                DisplayMessage("Cannot use Focus with this weapon!", MessageType.Error);
                return;
            }
            ...
            {
                DisplayMessage("Focus is empty!", MessageType.Warning);
            }
        }
        ...
        private void FoodScript_PerFrameDrawing(object sender, GTA.GraphicsEventArgs e)
        {
            ...
            if ((DateTime.Now - lastNpcScanTime).TotalSeconds >= 0.2)
            {
            ...
            if (inventoryOpen)
            {
                ...
                e.Graphics.DrawText(string.Format("|    Focus: {0}", deadeyeCore), xPosition + 100, yPosition, deadeyeColor);
            ...
            if (isDeadeyeActive)
            {
                ...
                e.Graphics.DrawText(string.Format("Focus: {0}", deadeyeCore), screenWidth / 2 - 50, 30, deadeyeUiColor);
            }
            ...
            if (shouldShowLootPrompt)
            {
                ...
                e.Graphics.DrawText("Press X to Loot", (int)(screenWidth * 0.5f) - 70, (int)(screenHeight * 0.8f), Color.White);
                //Game.DisplayText("Press X to Loot");
            }
        ...
        private void FoodScript_KeyDown(object sender, GTA.KeyEventArgs e)
        {
            ...
            if (e.Key == Keys.B && !isUsingItem && Game.LocalPlayer.CanControlCharacter && IsPlayerFree())
            {
                ToggleShop();
            }
            if (e.Key == Keys.X)
            ...
            if (e.Key == Keys.L && !isUsingItem && IsPlayerFree())
            {
                LootWorldObjects();
            }
        ...
        private void DrawCoreBars(GTA.GraphicsEventArgs e)
        {
            ...
            e.Graphics.DrawText("Focus", screenWidth * deadeyeBarX - 25, screenHeight * (barBaseY + barMaxHeight / 2) + 5, deadeyeColor);
        }
        ...
        private void StashHeldObject()
        {
            ...
            if (heldObject != null && heldObject.Exists())
            {
                ...
                if (foundItemKey != null)
                {
                    ...
                    {
                        DisplayMessage(string.Format("Cannot loot {0}, inventory is full!", itemDatabase[foundItemKey].Name), MessageType.Warning);
                    }
                }
                else
                {
                    DisplayMessage("This item cannot be stored.", MessageType.Error);
                }
            }
        }
        ...
        private void CompleteStashAction(string message, GTA.Object objectToStash)
        {
            ...
            DisplayMessage("Looted " + message + ".", MessageType.Success);
        }
        ...
        private string BuildEffectString(ItemInfo item)
        {
            ...
            {
                if (item.DeadeyeRestore != 0) effects.Add(string.Format("{0}{1} Focus", (item.DeadeyeRestore > 0 ? "+" : ""), item.DeadeyeRestore));
            ...
            if (item.DeadeyeRestore != 0) effects.Add(string.Format("{0} Focus", (item.DeadeyeRestore > 0 ? "+" : "") + item.DeadeyeRestore));
        ...
        private void DrawShopMenu(GTA.GraphicsEventArgs e)
        {
            switch (currentShopState)
            {
                ...
                case ShopState.Deadeye:
                    DrawShopItemsPage(e, "deadeye", "Liquors/Cigarettes", Color.Red);
                    break;
        ...
        private void ConfirmSellValuablesSafe()
        {
            ...
            int launderingFee = (int)(totalSaleValue * 0.30f);
        ...
        private void UpdateShopTotals()
        {
            ...
            foreach (var item in shoppingCart)
            {
                ...
                subtotal += itemDatabase[item.Key].Price * item.Value;
            }

            shippingFee = (int)Math.Round(subtotal * 0.20f);
        ...
        private void DrawInventoryMenu(GTA.GraphicsEventArgs e)
        {
            switch (currentState)
            {
                ...
                case InventoryState.Deadeye:
                    DrawInventoryItemsPage(e, "deadeye", "Liquors/Cigarettes", Color.Red);
                    break;
            ...
            var menuLines = new List<string>
    {
            ...
            else
            {
                ...
                foreach (string key in displayKeys)
                {
                    ...
                    foreach (var ing in info.Ingredients)
                    {
                        ...
                        string itemName = itemDatabase.ContainsKey(ing.Key) ? itemDatabase[ing.Key].Name : ing.Key;
                        ingredientParts.Add(string.Format("{0}/{1} {2}", owned, ing.Value, itemName));
                    }
        ...
        private int GetCraftableAmount(string combineKey)
        {
            ...
            {
                int currentAmount = inventory.ContainsKey(combo.ResultItemKey) ? inventory[combo.ResultItemKey] : 0;
                int spaceLeft = inventoryCapacity - currentAmount;
        ...
        private void PerformCombination(string combineKey)
        {
            ...
            if (!combineDatabase.ContainsKey(combineKey))
            {
                DisplayMessage("Recipe not found!", MessageType.Error);
                return;
            }
            ...
            if (combo.ResultItemKey != null)
            {
                if (!inventory.ContainsKey(combo.ResultItemKey))
                {
                    inventory[combo.ResultItemKey] = 0;
                }
                inventory[combo.ResultItemKey]++;
                
                if (!itemDatabase.ContainsKey(combo.ResultItemKey))
                {
                    DisplayMessage("Result item not found in database!", MessageType.Error);
                    return;
                }
            ...
            "amb@drink_can", "amb@bottle_create", "amb@kiosk", "amb@coffee_idle_m", "amb@nuts_idle"
                ...
                foreach (var ingredient in combo.Ingredients)
                {
                    if (!itemDatabase.ContainsKey(ingredient.Key))
                    {
                        continue;
                    }
        ...
        private void UseItem(string itemKey)
        {
            try
            {
                ...
                {
                    //if (Player.Character.isShooting || Game.isGameKeyPressed(GameKey.Attack) || ((Game.isGameKeyPressed(GameKey.Jump) || (Game.isGameKeyPressed(GameKey.Reload) || (Game.isGameKeyPressed(GameKey.Aim)))) && !player.Character.isInVehicle()))
                    // if (!Function.Call<bool>("IS_PLAYER_FREE_FOR_AMBIENT_TASK", Player.Index))
                    if (!IsPlayerFree())
                    {
                        wasCancelled = true;
                ...
                try
                {
                    ...
                    if (item.DeadeyeRestore != 0)
                    {
                        ...
                        changes.Add(string.Format("{0}Focus: {1}~w~", color, deadeyeCore));
                    }
        ...
        private void DrawShopSellValuablesPage(GTA.GraphicsEventArgs e)
        {
            ...
            e.Graphics.DrawText(string.Format("Backspace. Sell Safely (-30% Fee): {0}$", finalPayout), menuX + 250, infoY + 80, Color.LightGreen);
        ...
        private void CheckForLootableNpcs()
        {
            ...
            if (inventoryOpen || isShopOpen || isUsingItem || !IsPlayerFree())
            {
                shouldShowLootPrompt = false;
                return;
            }
